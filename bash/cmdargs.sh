...
#          Anything that wasn't recognized on the command line should still go into the myargs array
  ...
  case $1 in
    "-h" | "--help" )
      echo "you chose help!"
      echo ""
      echo "Usage: $0 [-h] [things to do]"
      exit
      ;;
    "-v" )
      echo "you chose verbose mode"
      verbose=1
      ;;
    "-d" )
      shift
      re='^[0-9]+$'
      if ! [[ $1 =~ $re ]] ; then
        echo "You need to specify a NUMBER for -d!" >&2; exit 1
      else
        debuglevel=$1
      fi
      ;;
...
if [[ $verbose == 1 ]] ; then
  ...
  echo ""
elif [[ ! -z "$debuglevel" ]] ; then
  echo ""
  echo "Debug mode is on! You issued debug level $debuglevel"
  echo ""
fi

printf "your list of arguments was:\n"
echo "${myargs[@]}"
