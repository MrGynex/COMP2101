...
#   - use the same kind of testing that is already in the script to only download the tarfile if you don't already have
# it and  to make sure the download and extraction commands work, then delete the local copy of the tarfile if the extraction was successful
...
#these commands are for checking if the tar file is present, and if not acquiring it
...
#tests to see if the tar file is present. If the file is present then it is extracted and the tarball is removed.
